...
- **Global pages** — Artifacts and Skills (sidebar rows, `data-testid="global-page"`). Full-main-region overlays belonging to no session, so they work from the home screen; any session activation (a lane row, New) closes them. Skills lives only here — browse, create, import/export and install into pi's global or project roots. The Artifacts page indexes every open session's artifacts; opening one jumps to its session with the per-session pane on it.
...
- **Empty states**: no workspace → warm onboarding card with folder picker; no sessions → "Describe a task…" hero; pi missing/outdated → setup screen with install command.
