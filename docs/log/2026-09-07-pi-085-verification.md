# pidex verified against pi 0.85.1, and the drift banner was 7 minors stale

Fable 5.1 was missing from the model menu. Not a pidex bug: the menu is pi's
...
question behind the question was whether pidex is actually verified on the pi
...
## What 0.85 actually changed for pidex
...
- **Response shapes**: `rpc-types.d.ts` and `json-event.d.ts` differ from
  0.84.1 only by the 0.84.2–0.84.4 changes pidex already reads (`usage` on
...
Of the release's own notes, one item is a straight win for pidex: 0.85.0 fixed
`abort` reporting success without cancelling an in-progress manual compaction.

`MIN_PI_VERSION` therefore stays at `0.84.1`. Nothing pidex needs is 0.85-only,
...
provider) and drives the surface pidex depends on. It runs one real model turn,
...
- All three extension-fed UI surfaces reported in on the status channel:
  `pidex-context-breakdown`, `pidex-mcp-status`, `mcp` (bundled extensions)
...
emitted by pi and handled nowhere in pidex. Pre-existing — all three exist in
