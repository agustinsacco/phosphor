...
describe('durable routine ledger', () => {
  ...
  it('deletes a routine with its history and hands back only the lanes it still owns', () => {
    const r = repo.save(input(), now)
    const other = repo.save(input({ name: 'Other' }), now)
    const kept = repo.manual(r.id, 'first', now)
    repo.claim(kept.id, now)
    repo.patchRun(kept.id, { sessionPath: '/s/kept.jsonl' })
    expect(() => repo.delete(r.id)).toThrow('Cancel')
    repo.finish(kept.id, 'finished', '', now)
    repo.promote(kept.id)
    const owned = repo.manual(r.id, 'second', now + 1)
    repo.claim(owned.id, now + 1)
    repo.patchRun(owned.id, { sessionPath: '/s/owned.jsonl' })
    repo.finish(owned.id, 'finished', '', now + 2)
    repo.manual(other.id, 'other', now)

    expect(repo.delete(r.id)).toEqual(['/s/owned.jsonl'])
    expect(() => repo.get(r.id)).toThrow('not found')
    expect(repo.history(r.id)).toHaveLength(0)
    expect(repo.history(other.id)).toHaveLength(1)
